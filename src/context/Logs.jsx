  ...
  const handleDownload = () => {
    ...
    if (isValidURL) {
      ...
      files.forEach((file) => {
        ...
        a.href = encodeURI(
          `http://localhost:3001/download?cluster=${clusterRef.current.value}&app=${appRef.current.value}&date=${dateRef.current.value}&time=${timeRef.current.value}&pod=${podRef.current.value}&files=${file}`
        );
        a.download = file;
